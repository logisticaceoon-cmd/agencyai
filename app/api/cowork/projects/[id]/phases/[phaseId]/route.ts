...
export async function GET(
  ...
  try {
    ...
    if (error || !data) {
      return NextResponse.json({ success: false, error: 'Fase no encontrada' }, { status: 404 })
    }

    return NextResponse.json({ success: true, data: { phase: taskToPhase(data) }, timestamp: new Date().toISOString() })
  } catch (err: any) {
...
export async function PATCH(
  ...
  try {
    ...
    if (error || !data) {
      return NextResponse.json({ success: false, error: error?.message || 'Fase no encontrada' }, { status: 404 })
    }
...
export async function DELETE(
  ...
  try {
    ...
    return NextResponse.json({ success: true, data: { message: 'Fase eliminada correctamente' }, timestamp: new Date().toISOString() })
  } catch (err: any) {
    return NextResponse.json({ error: err?.message || 'Error interno del servidor' }, { status: 500 })
  }
}
