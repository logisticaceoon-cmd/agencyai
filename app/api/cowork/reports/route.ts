...
export async function GET(request: Request) {
  try {
    ...
    if (error) {
      ...
      return NextResponse.json({ error: `Error fetching reports: ${error.message}` }, { status: 500 })
    }
...
export async function POST(request: Request) {
  try {
    ...
    if (error) {
      ...
      return NextResponse.json({ error: `Error creating report: ${error.message}` }, { status: 500 })
    }
