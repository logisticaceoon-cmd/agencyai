...
// Endpoint de setup v2 — usa exec_migration RPC (misma técnica que apply-migration)
...
export async function GET(request: Request) {
  ...
  const { error: e1 } = await supabase.rpc('exec_migration', {
    sql: `CREATE TABLE IF NOT EXISTS role_ai_context (
  ...
  results.create_index = e2 ? `ERROR: ${e2.message}` : 'OK'
  ...
  results.create_trigger_fn = e3 ? `ERROR: ${e3.message}` : 'OK'

  const { error: e4 } = await supabase.rpc('exec_migration', {
    sql: `
      DROP TRIGGER IF EXISTS trg_role_ai_updated ON role_ai_context;
  ...
  results.create_trigger = e4 ? `ERROR: ${e4.message}` : 'OK'

  // Reload PostgREST schema cache
  const { error: eN } = await supabase.rpc('exec_migration', {
    sql: `NOTIFY pgrst, 'reload schema'`
  }).single()
  results.notify_pgrst = eN ? `WARN: ${eN.message}` : 'OK'
...
- Si pide hacer algo que requiere permisos: "Para eso necesitás permisos adicionales. Contactá al administrador del workspace."`,
  ...
  for (const r of roles) {
    const { error } = await supabase
      ...
      .upsert({
        ...
        restricted_topics: r.restricted_topics,
        system_prompt_addition: r.system_prompt_addition,
        can_create_tasks: r.can_create_tasks,
        can_view_finances: r.can_view_finances,
        can_view_team: r.can_view_team,
        can_view_all_clients: r.can_view_all_clients,
        can_view_performance: r.can_view_performance,
        can_view_reports: r.can_view_reports,
        tone_instruction: r.tone_instruction,
      }, { onConflict: 'role' })
    results[`seed_${r.role}`] = error ? `ERROR: ${error.message}` : 'OK'
  }

  const allOk = Object.entries(results)
    .filter(([k]) => k !== 'notify_pgrst')
    .every(([, v]) => v === 'OK')
  ...
  return NextResponse.json({
    ...
    message: allOk
      ...
      : '⚠️ Algunos pasos fallaron — ver results para detalle',
  })
}
