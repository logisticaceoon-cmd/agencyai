...
// Workspace members to monitor — configured via TEAM_EMAILS env var (comma-separated)
const TEAM_EMAILS = (process.env.TEAM_EMAILS || '').split(',').filter(Boolean)
...
async function logToSupabase(supabase: any, period: string, count: number, summary: string) {
  ...
  const RAFAEL_ID = process.env.DEFAULT_USER_ID || ''
...
async function sendSummaryEmail(emails: EmailSummary[], summary: string, period: string) {
  ...
  try {
    ...
    await transporter.sendMail({
      ...
      to: process.env.GMAIL_USER || process.env.RAFAEL_EMAIL || '',
      subject,
      html,
    })
