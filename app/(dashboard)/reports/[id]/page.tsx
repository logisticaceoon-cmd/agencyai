  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <div className="flex items-center gap-3">
        <Link href="/reports" className="text-[var(--text-muted)] hover:text-slate-900">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-5">
          ...
          {(report.fileUrls || []).length > 0 && (
            <div className="rounded-xl border border-[var(--border-base)] bg-white p-5">
              <h2 className="text-sm font-semibold text-[var(--text-muted)] mb-3 uppercase tracking-wider flex items-center gap-2">
              ...
              <div className="space-y-2">
                {(report.fileUrls || []).map((url: string, i: number) => (
                  <a
                    ...
                    className="flex items-center gap-2 rounded-lg border border-[var(--border-base)] bg-slate-100 px-4 py-2.5 text-sm text-indigo-400 hover:bg-slate-200 transition-colors"
                  >
                    <Paperclip className="h-4 w-4" />
            ...
            <div className="rounded-xl border border-[var(--border-base)] bg-white p-5">
              ...
              {!showValidationInput ? (
                ...
                <div className="space-y-3">
                  <textarea rows={3} value={validationComment} onChange={(e) => setValidationComment(e.target.value)} placeholder="Comentarios de validación (opcional)..." className="w-full rounded-lg border border-[var(--border-base)] bg-slate-100 px-4 py-2.5 text-slate-900 placeholder-[var(--text-muted)] focus:border-indigo-500 focus:outline-none resize-none text-sm" />
                  <div className="flex gap-2">
                    ...
                    <button onClick={() => { setShowValidationInput(false); setPendingAction(null) }} className="rounded-lg border border-[var(--border-base)] px-4 py-2 text-sm text-[var(--text-muted)] hover:bg-slate-100 transition-colors">Cancelar</button>
                  </div>
                </div>
              )}
            </div>
          ...
          {report.validatedBy && (
            <div className={`rounded-xl border p-4 ${report.status === 'validated' ? 'border-green-500/30 bg-green-500/5' : report.status === 'rejected' ? 'border-red-500/30 bg-red-500/5' : 'border-yellow-500/30 bg-yellow-500/5'}`}>
              ...
              {report.validationComments && <p className="text-sm text-[var(--text-muted)] mt-1">{report.validationComments}</p>}
            </div>
          )}
          ...
          <div className="rounded-xl border border-[var(--border-base)] bg-white p-5">
            ...
            <div className="flex gap-2">
              <input type="text" value={comment} onChange={(e) => setComment(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && submitComment()} placeholder="Escribí un comentario..." className="flex-1 rounded-lg border border-[var(--border-base)] bg-slate-100 px-4 py-2 text-sm text-slate-900 placeholder-[var(--text-muted)] focus:border-indigo-500 focus:outline-none" />
        ...
        <div className="space-y-4">
          <div className="rounded-xl border border-[var(--border-base)] bg-white p-5 space-y-4">
            <div>
              <p className="text-xs text-[var(--text-secondary)] mb-1">Enviado por</p>
              <div className="flex items-center gap-2">
                <Avatar name={report.submittedBy?.fullName || 'Usuario'} avatarUrl={report.submittedBy?.avatarUrl} size="sm" />
                <div>
                  <p className="text-sm text-slate-900">{report.submittedBy?.fullName || 'Usuario'}</p>
                  {report.submittedBy?.department && <p className="text-xs text-[var(--text-secondary)]">{report.submittedBy.department}</p>}
                </div>
              </div>
            </div>
            <div>
              <p className="text-xs text-[var(--text-secondary)] mb-1">Fecha</p>
              <p className="text-sm text-slate-700">{formatDateTime(report.createdAt)}</p>
            </div>
            {report.client && (
              <div>
            ...
            {report.task && (
              <div>
                <p className="text-xs text-[var(--text-secondary)] mb-1">Tarea relacionada</p>
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-1">
                  {(report.tags || []).map((tag: string) => (
                    <span key={tag} className="rounded-full bg-slate-100 px-2 py-0.5 text-xs text-[var(--text-muted)]">{tag}</span>
                  ))}
                </div>
              </div>
            )}
