...
export default function SettingsPage() {
  ...
  useEffect(() => {
    loadOrg()
    loadTeam()
    // Auto-refresh equipo cada 30 segundos para detectar cuando Stephany acepta
              ...
              <>
                ...
                <div className="divide-y divide-slate-100">
                  {teamMembers.filter(m => m.status === 'invited').map(m => {
                    const displayName = m.name && m.name !== m.email.split('@')[0] ? m.name : m.email.split('@')[0]
                    const inviteUrl = `https://agencyai-iota.vercel.app/invite/${m.user_id.replace('invited_', '')}`
                    return (
