...
function getDaysRemainingBadge(deadline: string) {
  ...
  if (days < 0) return { label: `${Math.abs(days)}d vencida`, color: 'bg-[var(--red-light)] text-[var(--red)]' }
  if (days <= 3) return { label: days === 0 ? 'Hoy' : `${days}d`, color: 'bg-[var(--yellow-light)] text-[var(--yellow)]' }
...
const priorityLabel: Record<string, string> = { critical: 'Critica', high: 'Alta', medium: 'Media', low: 'Baja' }
    ...
    <div className="flex gap-6 h-[calc(100vh-56px-48px)]">
      ...
      <div className="flex-1 min-w-0 overflow-y-auto space-y-6 pr-1">
        ...
        <div className="grid grid-cols-1 xl:grid-cols-5 gap-5">
          <div className="xl:col-span-3 rounded-[var(--radius-lg)] border border-[var(--border-base)] bg-white p-5">
            ...
            {loading ? (
              ...
              <div className="space-y-1">
                {stats.pendingTasks.slice(0, 5).map((task) => {
                  ...
                  return (
                    <Link key={task.id} href={`/tasks/${task.id}`} className="flex items-center justify-between rounded-lg px-3 py-2 hover:bg-[var(--bg-subtle)] transition-colors group">
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-medium text-[var(--text-primary)] truncate group-hover:text-[var(--blue)] transition-colors">{task.title}</p>
                        <div className="flex items-center gap-2 mt-0.5">
                          {task.client && <span className="text-xs text-[var(--text-muted)]">{task.client.name}</span>}
                          <span className={cn('inline-flex items-center rounded-full px-2 py-0.5 text-[10px] font-semibold', priorityColor[task.priority] || priorityColor.medium)}>
                            {priorityLabel[task.priority] || task.priority}
                          </span>
                        </div>
                      </div>
                      <span className={cn('inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-semibold flex-shrink-0 ml-3', badge.color)}>{badge.label}</span>
                    </Link>
                  )
                })}
              </div>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-5 pb-6">
          <div className="rounded-[var(--radius-lg)] border border-[var(--border-base)] bg-white p-5">
            ...
            {!loading && stats && stats.recentClients.length > 0 ? (
              <div className="space-y-2">
                {stats.recentClients.map((client) => (
                  <Link key={client.id} href={`/clients/${client.id}`} className="flex items-center gap-3 rounded-lg p-2.5 hover:bg-[var(--bg-subtle)] transition-colors">
                    <div className={cn('flex h-9 w-9 items-center justify-center rounded-full text-xs font-semibold text-white', getAvatarColor(client.name))}>{getInitials(client.name)}</div>
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-medium text-[var(--text-primary)] truncate">{client.name}</p>
                      <p className="text-xs text-[var(--text-muted)]">{formatDate(client.createdAt)}</p>
                    </div>
                  </Link>
                ))}
              </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              {[
                ...
                <Link key={href} href={href} className="flex items-center gap-3 rounded-[var(--radius-md)] border border-[var(--border-base)] bg-white p-3 hover:border-[var(--border-strong)] hover:shadow-[var(--shadow-sm)] transition-all group">
                  ...
                  <div>
                    <p className="text-sm font-medium text-[var(--text-primary)] group-hover:text-[var(--blue)] transition-colors">{label}</p>
      ...
      {/* ── Right column: Agent Chat (35%) — desktop only ────────────── */}
      <div className="hidden md:flex w-[340px] xl:w-[380px] flex-shrink-0">
        <AgentChat />
      </div>
...
function MetricCard({ icon: Icon, label, value, footer, iconBg, iconColor, footerColor }: {
  icon: React.ElementType; label: string; value: number; footer: string; iconBg: string; iconColor: string; footerColor?: string
}) {
  return (
    <div className="metric-card">
      <div className="metric-header">
        <div className={cn('metric-icon', iconBg)}><Icon size={16} strokeWidth={1.5} className={iconColor} /></div>
        <span className="metric-label">{label}</span>
      </div>
      <div className="metric-value">{value}</div>
      <div className="metric-footer"><span className={cn('metric-period', footerColor)}>{footer}</span></div>
    </div>
  )
}
